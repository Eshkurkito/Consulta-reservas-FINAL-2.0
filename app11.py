...
# ---------- Cuadro de mando PRO (incluye Pace YoY) ----------
    ...
    if run_pro:
        ...
        # KPIs base
        _, tot = compute_kpis(raw, pd.to_datetime(cutoff_pro), pd.to_datetime(start_pro), pd.to_datetime(end_pro), inv_now, props_sel)
        occ, adr, revpar = tot["ocupacion_pct"], tot["adr"], tot["revpar"]

        # === ADR histórico (LY, LY-2, LY-3) con delta, semáforo e interpretación ===
        adr_hist = []  # [(label, adr_val)]
        if years_back > 0:
            for k in range(1, years_back + 1):
                ...
                _bp_k, tot_k = compute_kpis(
                    ...
                    period_end=end_k,
                    inventory_override=int(inv_now) if inv_now else None,  # para ADR no afecta
                    filter_props=props_sel
                )
                adr_k = float(tot_k["adr"])
                adr_hist.append((f"LY-{k}" if k > 1 else "LY", adr_k))

        # Tarjetas: ADR actual + históricos con delta vs LY
        ...
        # Gráfico comparativo barras (Actual vs históricos)
        if adr_hist:
            comp_df = pd.DataFrame(
                [("Actual", float(adr))] + adr_hist,
                columns=["Periodo", "ADR"]
            )
            st.bar_chart(comp_df.set_index("Periodo"), height=220)

        # Semáforo precio vs LY + interpretación
        ...
        if adr_ly and adr_ly > 0:
            ...
            if delta_pct >= band:
                ...
                insight = "Hay margen para mantener o subir en fechas pico, vigilando conversión."
            elif delta_pct <= -band:
                status = "🔴 Por debajo de LY"
                insight = "Revisar precio/visibilidad; posible sobre-restricción (mínimos/estancias)."
            else:
                status = "🟠 En línea con LY"
                insight = "Mantén estrategia y ajusta quirúrgicamente por DOW/eventos."
        ...
        # Health score (peso 40/30/30)
        health = 0
        health += min(occ / 80 * 40, 40)     # 80% occ ≈ 40/40
        health += min(adr / 100 * 30, 30)    # 100€ ADR ≈ 30/30 (ajusta a tu mercado)
        health += min(revpar / 60 * 30, 30)  # 60€ RevPAR ≈ 30/30 (ajusta)
        c1, c2, c3 = st.columns(3)
        c1.metric("Ocupación %", f"{occ:.1f}%")
        c2.metric("ADR medio", f"{adr:.2f} €")
        c3.metric("RevPAR", f"{revpar:.2f} €")
        st.progress(int(health), text=f"Health Score: {health:.0f}/100")

        # Forecast y pickup rápido
        forecast = pace_forecast_month(raw, pd.to_datetime(cutoff_pro), pd.to_datetime(start_pro), pd.to_datetime(end_pro), 2, 180, props_sel, inv_now)
        st.markdown(f"**Pickup necesario**: {forecast['pickup_needed_p50']:.0f} · Típico≈ {forecast['pickup_typ_p50']:.0f} (P75≈ {forecast['pickup_typ_p75']:.0f})")

        # --- NUEVO: Comparativa de ritmo (pace) vs LY ---
        ...
        if not base_pace.empty and not ly_pace.empty:
            # Alinear por D y construir dataframe comparativo
            ...
            if len(ly_vals) < len(comp):
                # Relleno seguro en caso de longitudes desiguales
                ly_vals = np.pad(ly_vals, (0, len(comp)-len(ly_vals)), constant_values=np.nan)
            comp["Occ % LY"] = ly_vals[:len(comp)]
            st.line_chart(comp.set_index("D")[["Occ % actual", "Occ % LY"]], height=280)

            # Evaluación rápida en hitos clave
            ...
            for i, D in enumerate(hitos):
                now_val = _val_at(base_pace, D, "ocupacion_pct")
                ly_val = _val_at(ly_pace, D, "ocupacion_pct")
                if np.isfinite(now_val) and np.isfinite(ly_val):
                    delta = now_val - ly_val
                    if delta > 5:
                        status = "🟢 Adelantado"
                    elif delta < -5:
                        status = "🔴 Retrasado"
                        ...
                        status = "🟠 En línea"
                    cols[i].metric(f"D-{D}", f"{now_val:.1f}% vs {ly_val:.1f}%", delta=f"{delta:+.1f} pp")
                    cols[i].write(status)
                else:
                    cols[i].metric(f"D-{D}", "—", delta="—")
                    cols[i].write("Sin datos")

        else:
            st.info("No se pudo calcular la comparativa de ritmo (faltan datos suficientes).")

        # ADR bands (posicionamiento de precio)
        dfb = raw[raw["Fecha alta"] <= pd.to_datetime(cutoff_pro)].dropna(subset=["Fecha entrada","Fecha salida"]).copy()

        # 👉 aplicar filtro de alojamientos si corresponde
        if props_sel:
        ...
        # filtrar solo estancias dentro del periodo analizado
        ...
        if not dfb.empty:
            ...
            if adr < q25:
                ...
                st.error("ADR por encima de P75 → riesgo de sobreprecio; revisa conversiones.")
            else:
        ...
        recs = []
        if occ < 60:
            recs.append("⚠️ Ocupación baja: prueba bajar mínimos/estancias, y abrir gaps cortos.")

        # Condiciones de ADR
        if np.isfinite(q25) and adr < q25:
            recs.append("💡 ADR bajo: no parece un problema de precio, sino de visibilidad. 👉 Aumenta campañas en OTAs, Google Hotel Ads o redes sociales.")
        elif np.isfinite(q75) and adr > q75:
            recs.append("📉 ADR alto: riesgo de sobreprecio. 👉 Considera bajar ADR o lanzar promos selectivas (última hora / estancias largas).")

        # Pickup retrasado con acciones según ADR
        if forecast['pickup_needed_p50'] > forecast['pickup_typ_p75']:
            if np.isfinite(q25) and adr < q25:
                recs.append("🚨 Pickup muy retrasado con ADR bajo → problema de visibilidad. 👉 Empuja campañas y revisa posicionamiento en portales.")
            elif np.isfinite(q75) and adr > q75:
                recs.append("🚨 Pickup muy retrasado con ADR alto → probable sobreprecio. 👉 Baja tarifas, abre disponibilidad y usa promos flash.")
            else:
                recs.append("🚨 Pickup muy retrasado → ritmo insuficiente. 👉 Revisa mínimos de estancia y activa campañas flash.")
